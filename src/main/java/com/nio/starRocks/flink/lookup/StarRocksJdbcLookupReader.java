...
package com.nio.starRocks.flink.lookup;
...
public class StarRocksJdbcLookupReader extends com.nio.starRocks.flink.lookup.StarRocksLookupReader {

    private static final Logger LOG = LoggerFactory.getLogger(StarRocksJdbcLookupReader.class);

    private ExecutionPool pool;

    private StarRocksRowConverter converter;

    private StarRocksRowConverter keyConverter;

    private LookupSchema schema;

    public StarRocksJdbcLookupReader(StarRocksOptions options, StarRocksLookupOptions lookupOptions, LookupSchema lookupSchema) {
        this.converter = new StarRocksRowConverter(lookupSchema.getFieldTypes());
        this.pool = new ExecutionPool(options, lookupOptions);
        this.schema = lookupSchema;
        this.keyConverter = buildKeyConvert();
    }

    private StarRocksRowConverter buildKeyConvert() {
        ...
        return new StarRocksRowConverter(keyTypes);
    }
