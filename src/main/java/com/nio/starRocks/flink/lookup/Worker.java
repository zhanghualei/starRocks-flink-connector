...
public class Worker implements Runnable {
    ...
    public Worker(AtomicBoolean started, StarRocksOptions options, StarRocksLookupOptions lookupOptions, int index) {
    ...
    public boolean offer(GetAction action) {
        if (exception.get() != null) {
            throw new StarRocksRuntimeException(exception.get());
        }
    ...
    private Map<RecordKey, List<Record>> executeQuery(String sql,
        ...
        for (int retry = 0; retry <= maxRetryTimes; retry++) {
            ...
            try {
                ...
                LOG.error(String.format("query starRocks error, retry times = %d", retry), e);
