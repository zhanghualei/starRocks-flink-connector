...
import com.nio.starRocks.flink.source.reader.StarRocksValueReader;
...
/**
 * InputFormat for {@link StarRocksDynamicTableSource}.
 */
@Internal
public class StarRocksRowDataInputFormat extends RichInputFormat<RowData, StarRocksTableInputSplit> implements ResultTypeQueryable<RowData> {
    ...
    private List<PartitionDefinition> starRocksPartitions;
    ...
    public StarRocksRowDataInputFormat(StarRocksOptions options,
                                   List<PartitionDefinition> starRocksPartitions,
                                   StarRocksReadOptions readOptions,
                                   RowType rowType) {
        this.options = options;
        this.starRocksPartitions = starRocksPartitions;
     ...
     */
    @Override
    public void open(StarRocksTableInputSplit inputSplit) throws IOException {
        valueReader = new StarRocksValueReader(inputSplit.partition, options, readOptions);
    ...
    @Override
    public StarRocksTableInputSplit[] createInputSplits(int minNumSplits) throws IOException {
        List<StarRocksTableInputSplit> starRocksSplits = new ArrayList<>();
        int splitNum = 0;
        for (PartitionDefinition partition : starRocksPartitions) {
            starRocksSplits.add(new StarRocksTableInputSplit(splitNum++, partition));
        }
        LOG.info("StarRocksTableInputSplit Num:{}", starRocksSplits.size());
        return starRocksSplits.toArray(new StarRocksTableInputSplit[0]);
    }

    @Override
    public InputSplitAssigner getInputSplitAssigner(StarRocksTableInputSplit[] inputSplits) {
    ...
    public static class Builder {
        private StarRocksOptions.Builder optionsBuilder;
        ...
        public Builder() {
            this.optionsBuilder = StarRocksOptions.builder();
        }
        ...
        public Builder setReadOptions(StarRocksReadOptions readOptions) {
            this.readOptions = readOptions;
            return this;
        }
        ...
        public StarRocksRowDataInputFormat build() {
            return new StarRocksRowDataInputFormat(
