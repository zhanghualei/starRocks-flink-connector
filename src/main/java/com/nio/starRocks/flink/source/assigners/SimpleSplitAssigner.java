...
// under the License.
package com.nio.starRocks.flink.source.assigners;
 ...
 **/
public class SimpleSplitAssigner implements StarRocksSplitAssigner {

    private final ArrayList<StarRocksSourceSplit> splits;

    public SimpleSplitAssigner(Collection<StarRocksSourceSplit> splits) {
        this.splits = new ArrayList<>(splits);
    }

    @Override
    public Optional<StarRocksSourceSplit> getNext(@Nullable String hostname) {
        final int size = splits.size();
        return size == 0 ? Optional.empty() : Optional.of(splits.remove(size - 1));
    }

    @Override
    public void addSplits(Collection<StarRocksSourceSplit> splits) {
