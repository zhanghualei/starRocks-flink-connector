...
import static com.nio.starRocks.flink.sink.writer.LoadConstants.JSON;
import static com.nio.starRocks.flink.sink.writer.LoadConstants.CSV;
import static com.nio.starRocks.flink.sink.writer.LoadConstants.STARROCKS_DELETE_SIGN;
import static com.nio.starRocks.flink.sink.writer.LoadConstants.JSON;
import static com.nio.starRocks.flink.sink.writer.LoadConstants.NULL_VALUE;

/**
 * Serializer for RowData.
 */
public class RowDataSerializer implements StarRocksRecordSerializer<RowData> {
    String[] fieldNames;
    String type;
    ...
    private RowDataSerializer(String[] fieldNames, DataType[] dataTypes, String type, String fieldDelimiter, boolean enableDelete) {
        ...
        this.rowConverter = new StarRocksRowConverter().setExternalConverter(dataTypes);
    }
    ...
    public String buildJsonString(RowData record, int maxIndex) throws IOException {
        ...
        if (enableDelete) {
            valueMap.put(STARROCKS_DELETE_SIGN, parseDeleteSign(record.getRowKind()));
        }
