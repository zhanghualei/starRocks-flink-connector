...
import static com.nio.starRocks.flink.sink.LoadStatus.LABEL_ALREADY_EXIST;
import static com.nio.starRocks.flink.sink.LoadStatus.SUCCESS;
import static com.nio.starRocks.flink.sink.ResponseUtil.LABEL_EXIST_PATTERN;
...
public class StarRocksStreamLoad implements Serializable {
    ...
    public StarRocksStreamLoad(String hostPort,
        ...
        this.user = starRocksOptions.getUsername();
        this.passwd = starRocksOptions.getPassword();
            ...
            try {
                ...
                if (LABEL_ALREADY_EXIST.equals(respContent.getStatus())) {
                    ...
                    if (JOB_EXIST_FINISHED.equals(respContent.getExistingJobStatus())) {
                        throw new StarRocksException("Load status is " + LABEL_ALREADY_EXIST + " and load job finished, " +
                    ...
                    if (matcher.find()) {
                        ...
                        throw new StarRocksException("Load Status is " + LABEL_ALREADY_EXIST + ", but no txnID associated with it!");
                    }
                } else {
    ...
    public void abortTransaction(long txnID) throws Exception {
        ...
        if (statusCode != 200 || response.getEntity() == null) {
            ...
            throw new StarRocksRuntimeException("Fail to abort transaction " + txnID + " with url " + abortUrlStr);
        }
        ...
        if (!SUCCESS.equals(res.get("status"))) {
            if (ResponseUtil.isCommitted(res.get("msg"))) {
                throw new StarRocksException("try abort committed transaction, " +
